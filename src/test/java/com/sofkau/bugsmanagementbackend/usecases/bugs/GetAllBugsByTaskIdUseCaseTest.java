...
@SpringBootTest
class GetAllBugsByTaskIdUseCaseTest {

    @SpyBean
    private GetAllBugsByTaskIdUseCase useCase;

    @MockBean
    private IBugsRepository repository;

    @Test
    void GetAllBugsByTaskIdTest (){
        ...
        Mockito.when(repository.findAllByTaskId(bugDTO.getTaskId())).thenReturn(Flux.just(bug, bug2));

        var resultFlux = useCase.apply("001-1");
